...
class DokumenRekomendasiSuratAhliWarisController {
  static async uploadDokumen(req, res) {
    try {
      if ((process.env.NODE_ENV || '').toLowerCase() === 'development') {
        console.log('REQ FILES SURAT AHLI WARIS:', req.files);
      }
      ...
      for (const file of files.filter((f) => String(f.fieldname || '').trim() !== 'file')) {
        ...
        if (!JENIS_DOKUMEN_SURAT_AHLI_WARIS.has(jenisDokumen)) {
          console.log('FIELD FILE TIDAK DIKENAL (SURAT AHLI WARIS):', jenisDokumen);
          continue;
        }
  ...
  static async hapusDokumen(req, res) {
    try {
      ...
      if (!jenisDokumen || !JENIS_DOKUMEN_SURAT_AHLI_WARIS.has(jenisDokumen)) {
        return R.badRequest(res, 'jenis_dokumen tidak valid');
      }
