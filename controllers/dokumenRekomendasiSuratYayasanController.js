...
class DokumenRekomendasiSuratYayasanController {
  static async uploadDokumen(req, res) {
    try {
      if ((process.env.NODE_ENV || '').toLowerCase() === 'development') {
        console.log('REQ FILES SURAT YAYASAN:', req.files);
      }
      ...
      for (const file of files.filter((f) => String(f.fieldname || '').trim() !== 'file')) {
        ...
        if (!JENIS_DOKUMEN_SURAT_YAYASAN.has(jenisDokumen)) {
          console.log('FIELD FILE TIDAK DIKENAL (SURAT YAYASAN):', jenisDokumen);
          continue;
        }
  ...
  static async hapusDokumen(req, res) {
    try {
      ...
      if (!jenisDokumen || !JENIS_DOKUMEN_SURAT_YAYASAN.has(jenisDokumen)) {
        return R.badRequest(res, 'jenis_dokumen tidak valid');
      }
