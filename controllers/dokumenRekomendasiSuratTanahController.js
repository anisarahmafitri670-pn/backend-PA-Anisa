...
const DokumenModel = require('../models/dokumenRekomendasiSuratTanahModel');
const { toPosixRelative, safeUnlinkRelativeUpload } = require('../utils/uploadFiles');

const JENIS_DOKUMEN_SURAT_TANAH = new Set([
...
class DokumenRekomendasiSuratTanahController {
  static async uploadDokumen(req, res) {
    try {
      if ((process.env.NODE_ENV || '').toLowerCase() === 'development') {
        console.log('REQ FILES SURAT TANAH:', req.files);
      }
      ...
      for (const file of files.filter((f) => String(f.fieldname || '').trim() !== 'file')) {
        ...
        if (!JENIS_DOKUMEN_SURAT_TANAH.has(jenisDokumen)) {
          console.log('FIELD FILE TIDAK DIKENAL (SURAT TANAH):', jenisDokumen);
          continue;
        }
  ...
  static async hapusDokumen(req, res) {
    try {
      ...
      if (!jenisDokumen || !JENIS_DOKUMEN_SURAT_TANAH.has(jenisDokumen)) {
        return R.badRequest(res, 'jenis_dokumen tidak valid');
      }
...
module.exports = { DokumenRekomendasiSuratTanahController, JENIS_DOKUMEN_SURAT_TANAH };
