...
describe('Iterasi 1 - Pengajuan Surat Tanah', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  test('berhasil membuat pengajuan dengan data valid', async () => {
    RekomendasiSuratTanahModel.savePengajuan.mockResolvedValue({
      success: true,
      id: 987
    });
    ...
    await RekomendasiSuratTanahController.buatPengajuan(req, res);

    expect(RekomendasiSuratTanahModel.savePengajuan).toHaveBeenCalledTimes(1);
    expect(RekomendasiSuratTanahModel.savePengajuan).toHaveBeenCalledWith({
      id_user: 1,
      nama_pemohon: 'Vivi',
      alamat: 'Bukit Sari',
      nik: '1409020105890006',
    ...
    expect(res.json).toHaveBeenCalledWith({
      success: true,
      message: 'Pengajuan rekomendasi surat tanah berhasil dibuat',
      data: {
        id_pengajuan: 987,
        id_user: 1,
  ...
  test.each([
    ...
    expect(RekomendasiSuratTanahModel.savePengajuan).not.toHaveBeenCalled();
