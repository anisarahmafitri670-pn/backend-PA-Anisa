...
describe('Iterasi 1 - Pengajuan Kartu Keluarga', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  test('berhasil membuat pengajuan dengan data valid', async () => {
    RekomendasiKartuKeluargaModel.savePengajuan.mockResolvedValue({
      success: true,
      id: 456
    });
    ...
    await RekomendasiKartuKeluargaController.buatPengajuan(req, res);

    expect(RekomendasiKartuKeluargaModel.savePengajuan).toHaveBeenCalledTimes(1);
    expect(RekomendasiKartuKeluargaModel.savePengajuan).toHaveBeenCalledWith({
      id_user: 1,
      nama_pemohon: 'Vivi',
      alamat: 'Bukit Sari',
      nik: '1409020105890006',
    ...
    expect(res.json).toHaveBeenCalledWith({
      success: true,
      message: 'Pengajuan rekomendasi kartu keluarga berhasil dibuat',
      data: {
        id_pengajuan: 456,
        id_user: 1,
  ...
  test.each([
    ...
    expect(RekomendasiKartuKeluargaModel.savePengajuan).not.toHaveBeenCalled();
